...
import type { FieldMap, FieldProps, GoogleOption, LocationInfo } from '@pages/organization/dto';
import { useAddressPickerMap } from '@pages/organization/hooks';
import { stripPostalCodeText } from '@pages/organization/utils';
import { AntButton, AntForm, AntModal, AntSpace, AntSpin, AntTypography } from '@shared/components';
...
export default function AddressPickerAutoComplete({
  ...
  const addressValue = AntForm.useWatch(fieldMap.address, form) ?? '';
  ...
  return (
    <>
      ...
      <AntModal
        ...
        footer={
          <AntSpace>
            <AntButton onClick={() => setOpen(false)}>取消</AntButton>
            <AntButton onClick={() => void locateCurrentPosition()} loading={locating}>
              定位到当前位置
            </AntButton>
            <AntButton onClick={() => void resolveCurrentCenter()} loading={resolvingCenter}>
              取地图中心点
            </AntButton>
            <AntButton
              type="primary"
      ...
      >
        <AntSpin spinning={booting}>
          ...
          {error ? (
            <AntTypography.Text type="danger" className={styles.errorText}>
              {map.error}
            </AntTypography.Text>
          ) : null}
        </AntSpin>
