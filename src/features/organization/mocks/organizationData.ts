...
export const generatePerformanceTestData = (
  scale: 'small' | 'medium' | 'large' | 'huge' = 'medium',
): OrganizationNode[] => {
  const configs = {
    small: { rootNodes: 50, maxDepth: 3, avgChildren: 2 },
    medium: { rootNodes: 200, maxDepth: 4, avgChildren: 3 },
    large: { rootNodes: 1000, maxDepth: 5, avgChildren: 4 },
    huge: { rootNodes: 5000, maxDepth: 6, avgChildren: 5 },
  };

  const config = configs[scale];
  return generateMassiveOrganizationTreeData(config.rootNodes, config.maxDepth, config.avgChildren);
};
...
export const getPerformanceStats = (
  ...
  const traverse = (nodes: OrganizationNode[], currentDepth: number) => {
  ...
  return {
    ...
    avgChildrenPerNode: totalNodes > 0 ? totalChildren / totalNodes : 0,
    leafNodes,
    rootNodeCount,
  };
};
