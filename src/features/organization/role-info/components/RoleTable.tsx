...
import { Button, Checkbox, Dropdown, Menu, Space, Table, Tag, Tooltip } from 'antd';

import { useLanguage } from '@/shared/hooks/useLanguage';

import type { Pagination, PlatformFilter, RoleRecord } from '../types';
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (platform: { app?: boolean; web?: boolean }) => {
        return (
          <Space size="small">
            <Tag color={platform.app ? 'blue' : 'default'}>App</Tag>
            <Tag color={platform.web ? 'green' : 'default'}>Web</Tag>
          </Space>
        );
      },
    },
    ...
    {
      title: (
        <Dropdown menu={{ items: [{ key: 'status', label: statusMenu }] }} placement="bottomLeft">
          <div className={styles.filterHeader}>
      ...
      render: (status: 'Normal' | 'Deleted') => (
        <Tag color={status === 'Normal' ? 'success' : 'default'}>{status}</Tag>
      ),
    },
    {
      title: 'Description',
