...
export const organizationTreeData: OrganizationNode[] = [
  {
    ...
    children: [
      ...
      {
        key: 'd008',
        title: 'Dealer 1-IJKMNMNMONMONMONMONMONMONMONMONMONMONIJKMNMNMONMONMONMONMONMONMONMONMONMON',
        type: 'dealer',
        canAdd: false,
        canDelete: true
      }
...
export const generateMassiveOrganizationTreeData = (
  ...
  const generateTitle = (level: number, index: number): string => {
    const prefixes = [
      'Pylontech', 'Tesla Energy', 'BYD', 'CATL', 'LG Chem',
      'Panasonic', 'Samsung SDI', '宁德时代', '比亚迪', '国轩高科'
    ];
    
    const dealerNames = [
      'Regional Dealer', 'Area Distributor', 'Zone Partner',
      'Territory Manager', 'District Representative',
      'Sales Partner', 'Business Associate', 'Market Specialist'
    ];
    
    const installerNames = [
      'Professional Installer', 'Certified Technician',
      'Master Electrician', 'Solar Specialist',
      'Energy Systems Expert', 'Renewable Tech',
      'Power Solutions Pro', 'Grid Integration Specialist'
    ];
  ...
  // 确定权限
  const getNodePermissions = (level: number, maxLevel: number): { canAdd: boolean; canDelete: boolean } => {
    if (level === 0) {
      // 根节点：不能删除，可以添加
  ...
  // 递归生成子节点
  const generateChildren = (parentId: string, level: number, remainingDepth: number): OrganizationNode[] => {
